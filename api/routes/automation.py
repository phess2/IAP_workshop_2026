...
async def trigger_make_posts():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error running makePosts automation: {str(e)}"
        )


@router.post("/reply-posts", response_model=AutomationResponse)
async def trigger_reply_posts(keywords: str = Query(..., description="Comma-separated keywords to search for (e.g., 'AI,startup,tech')")):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error running replyPosts automation: {str(e)}"
        )
